...
function dr_show_url($mod, $data, $page = NULL) {
	...
	if ($rule && $rule['show']) {
		...
		return dr_uri_prefix('rewrite', $mod, $cat, $fid).$url;
	}


	return dr_uri_prefix('cat_show_ext_php', $mod, $cat, $fid).'c=show&id='.$data['id'].($page ? '&page='.$page : '');
}
...
function dr_extend_url($mod, $data, $page = 0) {
	...
	if ($rule && $rule['extend']) {
        ...
        return dr_uri_prefix('rewrite', $mod, $cat, SITE_FID).$url;
	}


    return dr_uri_prefix('cat_show_ext_php', $mod, $cat, $fid) . 'c=extend&id='.$data['id'].($page ? '&page='.$page : '');
}
