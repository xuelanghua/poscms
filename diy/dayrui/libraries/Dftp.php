...
class Dftp {
	...
	public function connect($host, $username = '', $password = '', $port = '21', $pasv = false, $ssl = false, $timeout = 30) {
		$start = time();
		if ($ssl) {
			if (!$this->db = @ftp_ssl_connect($host, $port, $timeout)) {
				$this->err_code = 1;
				return false;
			}
		} else {
			if (!$this->db = @ftp_connect($host, $port, $timeout)) {
				$this->err_code = 1;
				return false;
			}
		}
		
		if (@ftp_login($this->db, $username, $password)) {
			if ($pasv) ftp_pasv($this->db, true);
			$this->db_time = time()-$start;
		   return true;
		} else {
			$this->err_code = 1;
		   return false;
	...
	public function put($remote, $local) {
		...
		if (@ftp_put($this->db, $remote, $local, $this->mode)) {
			return true;
		} else {
			$this->err_code = 7;
			return false;
	...
	public function f_delete($filename) {
		...
		if (@ftp_delete($this->db, $filename)) {
			return true;
		} else {
			$this->err_code = 4;
			return false;
	...
	public function nlist($dirname) {
		...
		if ($list = @ftp_nlist($this->db, $dirname)) {
			return $list;
		} else {
			$this->err_code = 5;
			return false;
