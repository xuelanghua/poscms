			...
			foreach ($this->site_info as $siteid => $t) {
				...
				if ($category) {
					...
					foreach ($category as $c) {
						if ($update == 1) {
							if ((!$c['child'] || $c['pcatpost']) && $c['mid']) {
								$c['total'] = $this->db->where('status', 9)->where('catid', $c['id'])->count_all_results($siteid.'_'.$c['mid']);
							} else {
								$c['total'] = 0;
							}
						} else {
				...
				if (isset($data['site'][$siteid]['use']) && $data['site'][$siteid]['use']) {
					...
					if ($config['nodb'] == 0) {
						...
						if ($category) {
							$CAT = $CAT_DIR = $fenzhan = $level = array();
							foreach ($category as $c) {
								$c['total'] = $update == 1 ? ((!$c['child'] || $data['setting']['pcatpost']) ? $this->db->where('status', 9)->where('catid', $c['id'])->count_all_results($siteid.'_'.$dirname) : 0) : $this->ci->get_cache('module-'.$siteid.'-'.$cdir, 'category', $c['id'], 'total');
								$pid = explode(',', $c['pids']);
								$level[] = substr_count($c['pids'], ',');
						...
						$cache['is_system'] = 1;
						// 兼容性处理，安装评论功能
						if (SYS_UPDATE && !$this->db->query("SHOW TABLES LIKE '".$this->db->dbprefix.$siteid.'_'.$dirname."_comment'")->row_array()) {
							$this->load->model('comment_model');
