			...
			<div class="tab-content">
				...
				<div class="tab-pane active" id="tab_0">
					<div class="form-body">
						...
						<div class="form-group r1">
							...
							<div class="col-md-9">
								<div class="radio-list">
									<label class="radio-inline"><input type="radio" <?php if ($data['id']) { ?>disabled <?php } else { ?>onclick="dr_tid(0)"<?php } ?> name="data[tid]" value="0" <?php echo dr_set_radio('tid', $data['tid'], '0', TRUE); ?> /> <?php echo fc_lang('单网页'); ?></label>
									<label class="radio-inline"><input type="radio" <?php if ($data['id']) { ?>disabled <?php } else { ?>onclick="dr_tid(1)"<?php } ?> name="data[tid]" value="1" <?php echo dr_set_radio('tid', $data['tid'], '1'); ?> /> <?php echo fc_lang('内容模块'); ?></label>
									<label class="radio-inline"><input type="radio" <?php if ($data['id']) { ?>disabled <?php } else { ?>onclick="dr_tid(2)"<?php } ?> name="data[tid]" value="2" <?php echo dr_set_radio('tid', $data['tid'], '2'); ?> /> <?php echo fc_lang('外部地址'); ?></label>
								</div>
							</div>
						</div>
						<div class="form-group dr_tid_1">
							<label class="col-md-2 control-label"><font color="red">*</font><?php echo fc_lang('共享模块'); ?>：</label>
							<div class="col-md-9">
								<label>
									<select <?php if ($data['id']) { ?>disabled <?php } ?> class="form-control" name="data[mid]">
										<option value=""> -- </option>
										<?php if (is_array($module)) { $count=count($module);foreach ($module as $t) {  if ($t['share']) { ?><option value="<?php echo $t['dirname']; ?>" <?php if ($t['dirname']==$data['mid']) { ?>selected<?php } ?>> <?php echo fc_lang($t['name']); ?> </option><?php }  } } ?>
									</select>
								</label>
								<label class="">&nbsp;&nbsp;<a href="<?php echo dr_url('module/index'); ?>" style="color:blue !important"><?php echo fc_lang('[共享模块管理]'); ?></a></label>
							</div>
						</div>
						<?php if (!$data['id']) { ?>
						<div class="form-group r1">
						...
						<?php if ($data['pid']==0) { ?>
						<div class="form-group dr_one" style="display:none">
							<label class="col-md-2 control-label"><?php echo fc_lang('绑定域名'); ?>：</label>
							<div class="col-md-9">
								<label><input class="form-control input-xlarge" type="text"  name="data[domain]" value="<?php echo $data['domain']; ?>"></label>
								<span class="help-block"><?php echo fc_lang('域名格式为: www.dayrui.com'); ?></span>
							</div>
						</div>
						<?php } ?>
						<div class="form-group r1">
						...
						<div class="form-group">
							...
							<div class="col-md-9">
								...
								<span class="help-block"><?php echo fc_lang('选择“是”时，将下级第一个栏目页数据作为当前的栏目页，只对父级栏目页有效'); ?></span>
							</div>
						</div>
						<?php echo $content; ?>
				...
				<div class="tab-pane" id="tab_1">
					<div class="form-body">
						<div class="form-group">
							...
							<div class="col-md-9">
								...
								<select class="form-control" name="data[setting][urlrule]">
									...
									<?php $return_u = $this->list_tag("action=cache name=urlrule  return=u"); if ($return_u) extract($return_u); $count_u=count($return_u); if (is_array($return_u)) { foreach ($return_u as $key_u=>$u) {  if ($u['type']==3) { ?><option value="<?php echo $u['id']; ?>" <?php if ($u['id']==$data['setting']['urlrule']) { ?>selected<?php } ?>> <?php echo $u['name']; ?> </option><?php }  } } ?>
								</select>
								</label>
						...
						<div class="form-group">
							<label class="col-md-2 control-label"><?php echo fc_lang('生成静态'); ?>：</label>
							<div class="col-md-9">
								<input type="checkbox" name="data[setting][html]" value="1" <?php if ($data['setting']['html']) { ?>checked<?php } ?> data-on-text="<?php echo fc_lang('开启'); ?>" data-off-text="<?php echo fc_lang('关闭'); ?>" data-on-color="success" data-off-color="danger" class="make-switch" data-size="small">
								<span class="help-block"><?php echo fc_lang('选择URL规则之后此项才会生效'); ?></span>
							</div>
						</div>
						...
						<div class="form-group">
							...
							<div class="col-md-9">
								<div class="checkbox-list">
									...
									<label class="checkbox-inline"><input name="syn[]" type="checkbox" value="6"> <?php echo fc_lang('生成静态'); ?></label>
								</div>
							</div>
						</div>
